...
const CICOPage: React.FC = () => {
  ...
  const [clockType, setClockType] = useState<"clock-in" | "clock-out" | null>(
    null,
  );
  ...
  const capture = (type: "clock-in" | "clock-out") => {
    if (
      (type === "clock-in" && hasClockedIn) ||
      (type === "clock-out" && hasClockedOut)
    ) {
      toast.error(
        `You have already ${type === "clock-in" ? "clocked in" : "clocked out"} today`,
        {
          position: "top-center",
        },
      );
    ...
    if (webcamRef.current) {
      ...
      if (imageSrc) {
        ...
        try {
          ...
          toast.success(
            `${clockType === "clock-in" ? "Clock-in" : "Clock-out"} successful!`,
            {
              position: "top-center",
            },
          );
        ...
        <Paper className="w-full max-w-lg p-6 rounded-lg shadow-lg bg-white flex flex-col items-center">
          ...
          {image && (
            <img
              src={image}
              alt="Captured"
              className="mt-6 w-full max-w-md border rounded-lg shadow-md"
            />
          )}
        </Paper>
        <ToastContainer />
